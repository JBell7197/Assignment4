...
//  MyClock
//
//  Created by Justin Bell on 11/10/24.
//
...
class ViewController: UIViewController, AVAudioPlayerDelegate {
    ...
    override func viewDidLoad() {
        countdownTimer.backgroundColor = UIColor.cyan
        countdownTimer.layer.borderWidth = 5.0
        countdownTimer.layer.borderColor = UIColor.black.cgColor
        countdownTimer.layer.cornerRadius = 5.0
        super.viewDidLoad()
        updateClock()
        startClockTimer()
    }
    ...
    func startTimer() {
        let countDownDuration = countdownTimer.countDownDuration
        print("Selected countdown duration: \(countDownDuration)")
        remainingTime = countDownDuration
        updateLabel2(time: countDownDuration)
